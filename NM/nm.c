...
void argsort(int* ind, double* a, int n){
  ...
  for(int i=0; i<n; i++){
    int min_index = i;
    for(int j=i; j<n; j++)
      if (a[ind[j]] < a[ind[min_index]]) min_index = j;
    int temp = ind[i]; ind[i] = ind[min_index]; ind[min_index] = temp;
  }
}
  ...
  while (iterations < maxiter){
    ...
    if (fxr < fsim[0]){
      ...
      if (fxr < fsim[(N+1)-2]){
        ...
        if (doshrink) {
          for (int j=1; j<N+1; j++) {
            for (int i=0; i<N; i++) sim(j,i) = sim(0,i) + sigma*(sim(j,i) - sim(0,i));
            fsim[j] = func(&sim(j,0), args);
          }
        }
      }
    }
